...
```typescript
// 推荐的向量分区数（根据数据量自动计算）
function recommendedVectorPartitions(rowCount: number): number {
  if (rowCount < 1000) return 1;
  if (rowCount < 10000) return Math.ceil(rowCount / 1000);
  return Math.ceil(Math.sqrt(rowCount));
}
```
...
MyMem 的检索不是简单向量搜索，而是一条完整的检索链路（`src/retriever.ts`）：

```
查询 → 意图分析 → 查询扩展 → [向量检索 ∥ BM25检索] → RRF 融合 → 时间增强 → 重要性加权 → 衰减增强 → 长度归一化 → 时间衰减 → 重排序 → MMR 多样性过滤 → 结果输出
```
...
| `hook-enhancements.ts` | Hook 级增强（8 种软干预） |
