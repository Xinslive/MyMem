...
describe("retriever BM25 query expansion gating", () => {
  ...
  it("keeps BM25 fallback results when query embedding fails", async () => {
    const { retriever } = createRetrieverHarness(
      {},
      {},
      {
  ...
  it("uses diagnostics from the ephemeral search retriever on --json --debug failures", async () => {
    ...
    createMemoryCLI({
      store: {
        hasFtsSupport: true,
        async vectorSearch() {
          throw new Error("vector search failed after embed failure");
        },
        async bm25Search() {
          throw new Error("bm25 search failed after embed failure");
        },
        async hasId() {
          return true;
        },
    ...
    assert.deepEqual(payload, {
      ...
      diagnostics: {
        ...
        errorMessage: "both vector and BM25 search failed: simulated embedded search failure, bm25 search failed after embed failure",
      },
    });
  });
});
