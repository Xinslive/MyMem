...
describe("runWithFileLock recovery", () => {
  ...
  it("no ENOENT after stale lock is unlinked (Issue #670 regression)", async () => {
    // Regression test for Issue #670:
    // Scenario: proactive cleanup removes a stale lock file (age > 5min),
    // then store() is called. With realpath:true (old behavior), proper-lockfile
    // calls fs.realpath() on the deleted file -> ENOENT.
    // Fix: realpath:false prevents this by skipping realpath() call.
    const { store, dir } = makeStore();
    const lockPath = join(dir, ".memory-write.lock");

    try {
      // Step 1: Simulate a stale lock artifact (like from a crashed process)
      writeFileSync(lockPath, "", "utf8");
      assert.strictEqual(existsSync(lockPath), true);

      // Step 2: Make it appear stale (mtime > 5 minutes old)
      // This is what proactive cleanup checks before unlinking
      const oldTime = new Date(Date.now() - 6 * 60 * 1000); // 6 minutes ago
      utimesSync(lockPath, oldTime, oldTime);

      // Step 3: Proactive cleanup unlinks the stale lock (see store.ts:221-230)
      rmSync(lockPath);
      assert.strictEqual(existsSync(lockPath), false);

      // Step 4: store() should succeed WITHOUT ENOENT
      // This fails with realpath:true because realpath() is called on deleted file
      const entry = await store.store(makeEntry(1));
      assert.ok(entry.id, "store() should succeed after stale lock unlink");
      assert.strictEqual(entry.text, "memory-1");
