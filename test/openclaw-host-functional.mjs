...
async function main() {
  const runDir = mkdtempSync(path.join(tmpdir(), "mymem-openclaw-host-"));
  const profile = `mymem-host-${Date.now()}`;
  ...
  try {
    ...
    const importOutput = await runOpenClaw(profile, ["mymem", "import", importFile, "--scope", "global"]);
    assert.match(importOutput, /Import completed: 2 imported, 0 skipped/);

    const listBeforeDelete = parseJsonOutput(await runOpenClaw(profile, ["mymem", "list", "--scope", "global", "--json"]));
    assert.equal(listBeforeDelete.length, 2);

    const searchOutput = parseJsonOutput(await runOpenClaw(profile, ["mymem", "search", "乌龙茶", "--scope", "global", "--json"]));
    assert.ok(searchOutput.length >= 1);
    assert.equal(searchOutput[0].entry.id, "11111111-1111-4111-8111-111111111111");

    const statsBeforeDelete = parseJsonOutput(await runOpenClaw(profile, ["mymem", "stats", "--scope", "global", "--json"]));
    assert.equal(statsBeforeDelete.memory.totalCount, 2);

    const exportOutput = await runOpenClaw(profile, ["mymem", "export", "--scope", "global", "--output", exportFile]);
    ...
    const deleteOutput = await runOpenClaw(profile, ["mymem", "delete", "22222222-2222-4222-8222-222222222222", "--scope", "global"]);
    assert.match(deleteOutput, /deleted successfully/i);

    const listAfterDelete = parseJsonOutput(await runOpenClaw(profile, ["mymem", "list", "--scope", "global", "--json"]));
    assert.equal(listAfterDelete.length, 1);
    assert.equal(listAfterDelete[0].id, "11111111-1111-4111-8111-111111111111");

    const statsAfterDelete = parseJsonOutput(await runOpenClaw(profile, ["mymem", "stats", "--scope", "global", "--json"]));
    assert.equal(statsAfterDelete.memory.totalCount, 1);

    const legacyPath = await createLegacyDb(runDir);
    const migrateOutput = await runOpenClaw(profile, ["mymem", "migrate", "run", "--source", legacyPath]);
    assert.match(migrateOutput, /Status:\s+Success/);
    assert.match(migrateOutput, /Migrated:\s+1/);

    const verifyOutput = await runOpenClaw(profile, ["mymem", "migrate", "verify", "--source", legacyPath]);
    assert.match(verifyOutput, /Valid:\s+Yes/);

    const listAfterMigrate = parseJsonOutput(await runOpenClaw(profile, ["mymem", "list", "--scope", "global", "--json"]));
    assert.equal(listAfterMigrate.length, 2);
    assert.ok(listAfterMigrate.some((entry) => entry.id === "legacy-1"));

    const statsAfterMigrate = parseJsonOutput(await runOpenClaw(profile, ["mymem", "stats", "--scope", "global", "--json"]));
