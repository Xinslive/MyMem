...
async function runAutoRecallHook(hooks, event, ctx) {
  for (const { handler } of hooks) {
    ...
    if (output?.prependContext?.includes("<relevant-memories>")) return output;
  }
  return undefined;
}
...
describe("recall text cleanup", () => {
  ...
  // Per-test mock retrieve function; set by each auto-recall test before register().
  let currentMockRetrieve = null;
  ...
  beforeEach(() => {
    ...
    // Patch prototype so the freshly-created singleton uses our mock.
    MemoryRetriever.prototype.retrieve = async function (params) {
      if (currentMockRetrieve) return currentMockRetrieve(params);
      return originalRetrieve.call(this, params);
    };
  });
  ...
  it("removes retrieval metadata from auto-recall injected text", async () => {
    // The singleton is reset in beforeEach. Set currentMockRetrieve so the
    // freshly-created singleton uses our mock data.
    const mockResults = makeResults();
    currentMockRetrieve = () => mockResults;
    ...
    assert.ok(hooks.length >= 1, "expected at least one before_prompt_build hook for this config");
  ...
  it("uses ingress raw text across session-key derived conversation keys", async () => {
    const { api, eventHandlers } = createHookApiHarness();
    let capturedQuery = "";
    ...
    const [{ handler: messageReceivedHook }] = eventHandlers.get("message_received") || [];
    const [{ handler: beforePromptBuildHook }] = eventHandlers.get("before_prompt_build") || [];
    assert.ok(messageReceivedHook, "expected message_received hook");
    assert.ok(beforePromptBuildHook, "expected before_prompt_build hook");

    messageReceivedHook(
      { content: "Ingress-specific raw query" },
      { channelId: "chan-1", conversationId: "conv-42" },
    );

    await beforePromptBuildHook(
      { prompt: "Fallback prompt should not be used" },
      { sessionId: "session-1", sessionKey: "agent:main:chan-1:conv-42", agentId: "main" },
    );

    assert.equal(capturedQuery, "Ingress-specific raw query");
  });

  it("accepts structured message_received content without breaking recall query caching", async () => {
    let capturedQuery = "";
    currentMockRetrieve = (params = {}) => {
      capturedQuery = params.query;
      return makeResults();
    };
    ...
    const messageHandlers = harness.eventHandlers.get("message_received") || [];
    assert.ok(messageHandlers.length >= 1, "expected message_received hooks to be registered");

    for (const { handler } of messageHandlers) {
      await handler(
        {
          from: "discord:channel:1",
          content: [{ type: "text", text: "Structured ingress query" }],
        },
        {
          channelId: "discord",
    ...
    await autoRecallHook(
      { prompt: "Fallback prompt should not be used" },
      { sessionId: "structured-session", sessionKey: "agent:main:discord:channel:1", agentId: "main" },
    );

    assert.equal(capturedQuery, "Structured ingress query");
  });

  it("uses entry timestamp for tier maintenance when metadata lacks created_at", async () => {
    const { api, eventHandlers } = createHookApiHarness();
    ...
    registerAutoRecallHook({
      ...
      tierManager: {
        evaluateAll(memories) {
          capturedCreatedAt = memories[0]?.createdAt ?? 0;
          return [];
        },
      },
    });
    ...
    await beforePromptBuildHook(
      { prompt: "Please recall the old durable memory." },
      { sessionId: "tier-maintenance", sessionKey: "agent:main:chan-1:conv-42", agentId: "main" },
    );
  ...
  it("applies auto-recall item/char budgets before injecting context", async () => {
    // Singleton is reset in beforeEach; set currentMockRetrieve so the
    // freshly-created singleton uses our mock data.
  ...
  function makeAutoRecallHarness(workspaceDir, mockResults, extraConfig = {}) {
    // The singleton is reset and MemoryRetriever.prototype is patched in beforeEach
    // to call currentMockRetrieve.  Set it here so the freshly-created singleton
    // uses our mock data instead of querying the real database.
    currentMockRetrieve = () => mockResults;
