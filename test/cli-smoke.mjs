...
async function runCliSmoke() {
  const workDir = mkdtempSync(path.join(tmpdir(), "mymem-smoke-"));
  ...
  await program.parseAsync(["node", "openclaw", "mymem", "version"]);
  ...
  const staleStore = new MemoryStore({
    dbPath: path.join(workDir, "stale-db"),
    vectorDim: 4,
  });
  await staleStore.importEntry({
  ...
  const repairProgram = new Command();
  repairProgram.exitOverride();
  createMemoryCLI({
    store: staleStore,
    retriever: { retrieve: async () => [] },
    scopeManager: { getDefaultScope: () => "global" },
  ...
  const repairDryRunOutput = await captureStdout(async () => {
  ...
  assert.match(repairDryRunOutput, /Found 1 stale entries/);
  assert.match(repairDryRunOutput, /Dry run complete/);

  const repairOutput = await captureStdout(async () => {
    await repairProgram.parseAsync([
      "node",
      "openclaw",
      "mymem",
