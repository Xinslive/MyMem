...
describe("mymem auth", () => {
  ...
  it("round-trips a dedicated llm api-key config through OAuth login/logout", async () => {
    ...
    const pluginConfig = updatedConfig.plugins.entries["mymem"].config;
    ...
    assert.match(output, /Updated mymem config: llm.auth=oauth, llm.oauthProvider=openai-codex/);
    ...
    const restoredPluginConfig = restoredConfig.plugins.entries["mymem"].config;
    assert.deepEqual(restoredPluginConfig.llm, originalLlmConfig);

    const logoutOutput = logoutLogs.join("\n");
    assert.match(logoutOutput, /Updated mymem config: llm.auth=api-key/);
  });
  ...
  it("defaults the OAuth file to the plugin-scoped path under OPENCLAW_HOME", async () => {
    ...
    const configPath = path.join(tempDir, "openclaw.json");
    const oauthPath = path.join(process.env.OPENCLAW_HOME, ".mymem", "oauth.json");
    const backupPath = getBackupPath(oauthPath);
    writeFileSync(configPath, JSON.stringify({
      plugins: {
        entries: {
  ...
  it("resolves stored relative oauthPath against the config location during logout", async () => {
    ...
    const storedOauthPath = ".mymem/oauth.json";
    const actualOauthPath = path.join(workspaceDir, ".mymem", "oauth.json");
  ...
  it("removes llm config on logout when only OAuth-generated fields remain and no backup exists", async () => {
    ...
    const configPath = path.join(workspaceDir, "openclaw.json");
    const oauthPath = path.join(workspaceDir, ".mymem", "oauth.json");
