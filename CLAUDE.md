...
- Tools: `src/tools.ts` is the runtime tool registration barrel. The current plugin manifest exposes `mymem_recall`, `mymem_update`, and `mymem_doctor`; other management/update modules remain in source for CLI, tests, or compatibility paths. As of 2026-07-21 the prior experimental management tools (`mymem_stats`, `mymem_list`, `mymem_promote`, etc.) and their `enableManagementTools` config flag were removed — `src/tools-management.ts`, `src/tools-store.ts`, `src/tools-forget.ts` no longer exist.
