...
import { TEMPORAL_VERSIONED_CATEGORIES, type MemoryCategory } from "./memory-categories.js";
...
const fallbackToolLogger: Pick<OpenClawPluginApi["logger"], "warn"> = {
...
function isLegacyMemoryCategory(category: string): category is (typeof MEMORY_CATEGORIES)[number] {
  return (MEMORY_CATEGORIES as readonly string[]).includes(category);
}

function toLegacyMemoryCategory(category: string): (typeof MEMORY_CATEGORIES)[number] | undefined {
  return isLegacyMemoryCategory(category) ? category : undefined;
}

function deriveManualMemoryCategory(category: string, text: string): MemoryCategory {
...
interface LearningBacklogEntry {
  id: string;
...
function extractLearningSection(entry: string, heading: string): string {
  const escapedHeading = escapeRegExp(heading);
  const match = entry.match(new RegExp(`### ${escapedHeading}\\n([\\s\\S]*?)(?=\\n### |\\n---|$)`, "m"));
  return (match?.[1] ?? "").trim();
}

function extractLearningField(entry: string, field: string): string {
  const escapedField = escapeRegExp(field);
  const match = entry.match(new RegExp(`\\*\\*${escapedField}\\*\\*:\\s*(.+)`, "i"));
  return (match?.[1] ?? "").trim();
}

function parseLearningBacklogEntries(file: "LEARNINGS.md" | "ERRORS.md", content: string): LearningBacklogEntry[] {
...
function normalizeDistillRuleText(text: string): string {
  return text
    .replace(/\s+/g, " ")
    .replace(/^[-*]\s+/, "")
...
function buildSelfImprovementDistillPatch(entries: LearningBacklogEntry[], targetFile: string): string {
