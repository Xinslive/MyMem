...
import { RecencyEngine, type RecencyEntry } from "./recency-engine.js";
...
const fallbackRetrieverLogger: Pick<Logger, "debug" | "warn"> = {
...
export class MemoryRetriever {
  ...
  private async vectorOnlyRetrieval(
    ...
    try {
      ...
      if (this.decayEngine) {
        ...
        temporallyRanked = this.applyRecencyComposite(mapped);
        if (diagnostics) diagnostics.stageCounts.afterRecency = temporallyRanked.length;
        if (diagnostics) diagnostics.stageCounts.afterImportance = temporallyRanked.length;
      } else {
        temporallyRanked = this.applyRecencyBoost(mapped);
        if (diagnostics) diagnostics.stageCounts.afterRecency = temporallyRanked.length;
        temporallyRanked = this.applyImportanceWeight(temporallyRanked);
        if (diagnostics) diagnostics.stageCounts.afterImportance = temporallyRanked.length;
      }
  ...
  private async hybridRetrieval(
    ...
    try {
      ...
      if (this.config.rerank !== "none") {
        ...
        const t3 = Date.now();
        reranked = await this.rerankResults(query, queryVector, rerankInput, signal);
      ...
      if (this.decayEngine) {
        ...
        temporallyRanked = this.applyRecencyComposite(reranked);
        trace?.endStage(temporallyRanked.map((r) => r.entry.id), temporallyRanked.map((r) => r.score));
        if (diagnostics) {
          diagnostics.stageCounts.afterRecency = temporallyRanked.length;
          diagnostics.stageCounts.afterImportance = temporallyRanked.length;
  ...
  private async fuseResults(
    ...
    if (ghostCheckIds.length > 0) {
      const hasIds = (this.store as BatchIdStore).hasIds;
      if (typeof hasIds === "function") {
        try {
          const existingIds = await resolveUnlessAborted(hasIds.call(this.store, ghostCheckIds), signal);
          throwIfAborted(signal);
          ...
          this.logger.debug(`[Retriever] batch hasIds check failed: ${err}`);
        }
      } else {
        const existenceResults = await resolveUnlessAborted(
          Promise.allSettled(
            ghostCheckIds.map(async (id) => {
              throwIfAborted(signal);
              return { id, exists: await this.store.hasId(id) };
            }),
          ),
          signal,
        );
        ...
        for (let index = 0; index < existenceResults.length; index++) {
          ...
          if (result.status === "fulfilled") {
            ...
            this.logger.debug(`[Retriever] hasId check failed for ${id}: ${result.reason}`);
          }
        }
      }
    }
  ...
  private async rerankResults(
    ...
    if (this.config.rerank === "cross-encoder" && hasApiKey && this.config.rerankModel && this.config.rerankEndpoint) {
      try {
        const model = this.config.rerankModel;
        const endpoint = this.config.rerankEndpoint;
        ...
        if (error instanceof Error && error.name === "AbortError") {
          ...
          const wasExternalAbort = externalSignal?.aborted === true;
          if (wasExternalAbort) {
            ...
            this.logger.warn(`Rerank API timed out (${this.config.rerankTimeoutMs ?? 5000}ms), falling back to cosine`);
          }
        } else {
          this.logger.warn("Rerank API failed, falling back to cosine:", error);
        }
      }
    }
  ...
  private applyRecencyComposite(results: RetrievalResult[]): RetrievalResult[] {
    if (!this.recencyEngine || results.length === 0) return results;
    const byId = new Map(results.map((result) => [result.entry.id, result]));
    return this.recencyEngine
