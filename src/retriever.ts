    ...
    if (ghostCheckIds.length > 0) {
      ...
      if (typeof hasIds === "function") {
        try {
          ...
          console.debug(`[Retriever] batch hasIds check failed: ${err}`);
        }
      } else {
        ...
        for (let index = 0; index < existenceResults.length; index++) {
          ...
          if (result.status === "fulfilled") {
            ...
            console.debug(`[Retriever] hasId check failed for ${id}: ${result.reason}`);
          }
        }
      }
    }
  ...
  private async recordAccessAndMaybeTransition(results: RetrievalResult[]): Promise<void> {
    ...
    for (const r of toUpdate) {
      ...
      try {
        ...
        console.debug(`[Retriever] tier metadata update failed for ${r.entry.id}: ${err}`);
      }
    }
  }
