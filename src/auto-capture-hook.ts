    ...
    const backgroundRun = (async () => {
      try {
        ...
        if (smartExtractor) {
          ...
          if (cleanTexts.length >= minMessages) {
            ...
            if ((stats.boundarySkipped ?? 0) > 0) {
              api.logger.info(`mymem: smart extraction skipped ${stats.boundarySkipped} USER.md-exclusive candidate(s) for agent ${agentId}; continuing to regex fallback for non-boundary texts`);
            }
          }
        }
        ...
        if (toCapture.length === 0) {
          api.logger.info(`mymem: regex fallback found 0 capturable texts for agent ${agentId}`);
          return;
        }

        api.logger.info(`mymem: regex fallback found ${toCapture.length} capturable text(s) for agent ${agentId}`);

        let stored = 0;
        for (const text of toCapture.slice(0, 2)) {
          if (isUserMdExclusiveMemory({ text }, config.workspaceBoundary)) {
            api.logger.info(`mymem: skipped USER.md-exclusive auto-capture text for agent ${agentId}`);
            continue;
          }
