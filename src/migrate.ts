/**
 * Migration Utilities
 * Migrates data from old memory-lancedb plugin to mymem
 */
...
const silentMigrationLogger: Pick<Logger, "info" | "warn"> = {
  info: () => {},
  warn: () => {},
};
...
export class MemoryMigrator {
  ...
  async migrate(options: MigrationOptions = {}): Promise<MigrationResult> {
    ...
    try {
      ...
      logger.info(`Migrating from: ${sourceDbPath}`);

      // Load legacy data
      const legacyEntries = await this.loadLegacyData(sourceDbPath, undefined, logger);
      ...
      logger.info(`Found ${legacyEntries.length} entries to migrate`);
  ...
  private async loadLegacyData(
    sourceDbPath: string,
    limit?: number,
    logger: Pick<Logger, "warn"> = silentMigrationLogger,
    ...
    try {
      ...
      logger.warn(`Failed to load legacy data: ${error}`);
      return [];
    }
  }
  ...
  private async migrateEntries(
    ...
    for (const legacy of legacyEntries) {
      try {
        ...
        // Include memory_category so isLegacyMemory() doesn't flag these as legacy.
        const newEntry: MemoryEntry = {
          ...
          metadata: JSON.stringify({
            memory_category: reverseMapLegacyCategory(legacy.category, legacy.text),
        ...
        if (migrated % 100 === 0) {
          logger.info(`Migrated ${migrated}/${legacyEntries.length} entries...`);
        }
