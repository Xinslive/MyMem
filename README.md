...
只绑定本机地址，不会暴露到外网。管理台 API 需要本地 token 解锁；自动启动或手动启动时会使用 `.dashboard-token` 文件，并在日志中提示形如 `http://127.0.0.1:1314/?token=$(cat '.../.dashboard-token')` 的打开方式。首次带 token 访问后，页面会设置本机 HttpOnly cookie 并从地址栏移除 token。
...
通常不需要手动运行；正常启用插件后管理台随 MyMem 一起启动。端口被占用时会跳过启动并继续正常工作。手动启动时 CLI 会打印 token 文件路径和解锁 URL。
